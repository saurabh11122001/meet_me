...
const ChatPage = () => {
  ...
  const {
    ...
    getReceiver,
    setNotification,
    notification,chatLoader,setChatLoader
    ...
    if (message.trim()) {
      try {
        ...
        if (response.ok) {
          ...
          socket.emit("send", {
            message: { sender: user._id, receiver: id, content: message , id:data.message_id},
            id
          });
          setMessage("");
        }
  ...
  const onChange = (e) => {
    ...
    setTimeout(() => {
      ...
      if (time_diff >= timerLength && typing) {
        socket.emit("stoptyping", { id, sender: user._id });
        setTyping(false);
      }
    }, timerLength);
  ...
  const handleDeleteMessage = (messageId, index) => {
    ...
    console.log("Delete ID set:", messageId);
  };

  const deleteMessage = async () => {
    try {
    
      let response = await fetch(`${process.env.REACT_APP_HOST}/message/delete/${deleteId}`, {
        method: 'GET',
        credentials: 'include'
      });
      if (response.ok) {
        console.log("Message Deleted");
        setReceivedMessages((prevMessages) => prevMessages.filter((msg, idx) => idx !== Index));
        setDeleteId(null);
     ...
     <div className="relative flex font-sans flex-col h-screen max-w-md mx-auto border p-4 bg-white">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {!messageSelected ? (
            ...
            <MdDelete onClick={deleteMessage} />
          )}
        </div>
        <div className="flex-1 overflow-y-auto w-full"ref={chatContainerRef}>
                ...
                {receivedMessage.map((message, index) => {
                  ...
                  return (
                    <div
                      ...
                      } ${messageSelected && index === Index ? 'bg-blue-400' : ''}`}
                    >
                      <div onClick={() => handleDeleteMessage(!message.id?message._id:message.id, index)}
                        className={`${
                          isCurrentUser
      ...
      {chatLoader?<div className="absolute h-full w-full bg-gray-200 top-0 flex justify-center items-center">  
